...
Numbers are the one exception. `mintId()` in [number.ts](packages/torph/src/lib/text-morph/utils/number.ts) issues `\u0000n<counter>`: text IDs are derived from the text, so a NULL prefix cannot collide with one, and a counter that only climbs cannot collide with an ID a number is still carrying from an earlier morph. Elements are the third: `flattenContent()` in [content.ts](packages/torph/src/lib/text-morph/utils/content.ts) issues `\u0000e<key>`, where the NULL prefix keeps it off text IDs and the discriminator off a number's. A fourth has to argue for its own disjointness the same way.

**A value is text and elements**, not only a string. `update()` takes an element to read, a string, a number, or the `ContentPart[]` a wrapper builds. An element child is atomic — it never splits, never pairs with a word, and enters, exits and travels as one thing, keyed by `data-torph-key`. The engine reparents what it places and React may not follow its own nodes across that move, so the React wrapper renders element children through portals into containers it created outside React, and holds a dropped child for one morph so it animates away rather than blinking out. Vue and Svelte still take `text` only.

An inline formatting tag with text in it (`b`, `strong`, `em`, `code`…, never `span` — too often a thing) is the exception: it is walked *through* rather than treated as a thing, and carried as a `Format` on the text inside it, so a word can gain or lose emphasis and still morph. A `data-torph-key` on one says the author means the element itself, and `data-torph-format` the reverse. A keyed element carrying `data-torph-interactive` stays in the accessibility tree and out of the plain-text copy — it announces itself, so the copy would say it twice. Word boundaries fall on format changes, so both sides of the diff agree on the words.
